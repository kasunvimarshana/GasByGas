<?php

return [
    'cache_enabled' => env('NAVIGATION_CACHE_ENABLED', false),
    'cache_ttl' => 3600, // Cache duration in seconds (1 hour)
    'default_permissions' => ['view'],
    'items' => [
        'Dashboard' => [
            'title' => 'Dashboard',
            'icon' => 'bi bi-speedometer me-1',
            'route' => null,
            'parent_id' => null,
            'order' => 0,
            'parameters' => [],
            'permission' => 'dashboard',
            'types' => null,
            'is_active' => true,
        ],
        'User' => [
            'title' => 'User',
            'icon' => 'bi bi-person-circle me-1',
            'route' => null,
            'parent_id' => null,
            'order' => 0,
            'parameters' => [],
            'permission' => 'users',
            'types' => null,
            'is_active' => true,
            'children' => [
                'User List' => [
                    'title' => 'User List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'users.index',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'users.index',
                    'types' => null,
                    'is_active' => true,
                ],
                'Company User List' => [
                    'title' => 'Company User List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'users.index-for-company',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'users.index-for-company',
                    'types' => null,
                    'is_active' => true,
                ],
                'User Create' => [
                    'title' => 'User Create',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'users.create',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'users.create',
                    'types' => null,
                    'is_active' => true,
                ],
                'Company User Create' => [
                    'title' => 'Company User Create',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'users.create-for-company',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'users.create-for-company',
                    'types' => null,
                    'is_active' => true,
                ],
                'User Edit' => [
                    'title' => 'User Edit',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'users.edit',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['user' => 0],
                    'permission' => 'users.edit',
                    'types' => null,
                    'is_active' => false,
                ],
                'User Show' => [
                    'title' => 'User Show',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'users.show',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['user' => 0],
                    'permission' => 'users.show',
                    'types' => null,
                    'is_active' => false,
                ],
            ]
        ],
        'Company' => [
            'title' => 'Company',
            'icon' => 'bi bi-person-circle me-1',
            'route' => null,
            'parent_id' => null,
            'order' => 0,
            'parameters' => [],
            'permission' => 'companies',
            'types' => null,
            'is_active' => true,
            'children' => [
                'Company List' => [
                    'title' => 'Company List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'companies.index',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'companies.index',
                    'types' => null,
                    'is_active' => true,
                ],
                'Company Create' => [
                    'title' => 'Company Create',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'companies.create',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'companies.create',
                    'types' => null,
                    'is_active' => true,
                ],
                'Company Edit' => [
                    'title' => 'Company Edit',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'companies.edit',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['company' => 0],
                    'permission' => 'companies.edit',
                    'types' => null,
                    'is_active' => false,
                ],
                'Company Show' => [
                    'title' => 'Company Show',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'companies.show',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['company' => 0],
                    'permission' => 'companies.show',
                    'types' => null,
                    'is_active' => false,
                ],
            ]
        ],
        'Product' => [
            'title' => 'Product',
            'icon' => 'bi bi-person-circle me-1',
            'route' => null,
            'parent_id' => null,
            'order' => 0,
            'parameters' => [],
            'permission' => 'products',
            'types' => null,
            'is_active' => true,
            'children' => [
                'Product List' => [
                    'title' => 'Product List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'products.index',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'products.index',
                    'types' => null,
                    'is_active' => true,
                ],
                'Product Create' => [
                    'title' => 'Product Create',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'products.create',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'products.create',
                    'types' => null,
                    'is_active' => true,
                ],
                'Product Edit' => [
                    'title' => 'Product Edit',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'products.edit',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['product' => 0],
                    'permission' => 'products.edit',
                    'types' => null,
                    'is_active' => false,
                ],
                'Product Show' => [
                    'title' => 'Product Show',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'products.show',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['product' => 0],
                    'permission' => 'products.show',
                    'types' => null,
                    'is_active' => false,
                ],
            ]
        ],
        'Stock' => [
            'title' => 'Stock',
            'icon' => 'bi bi-person-circle me-1',
            'route' => null,
            'parent_id' => null,
            'order' => 0,
            'parameters' => [],
            'permission' => 'stocks',
            'types' => null,
            'is_active' => true,
            'children' => [
                'Stock List' => [
                    'title' => 'Stock List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'stocks.index',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'stocks.index',
                    'types' => null,
                    'is_active' => true,
                ],
                'Stock Create' => [
                    'title' => 'Stock Create',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'stocks.create',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'stocks.create',
                    'types' => null,
                    'is_active' => true,
                ],
                'Stock Edit' => [
                    'title' => 'Stock Edit',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'stocks.edit',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['stock' => 0],
                    'permission' => 'stocks.edit',
                    'types' => null,
                    'is_active' => false,
                ],
                'Stock Show' => [
                    'title' => 'Stock Show',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'stocks.show',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['stock' => 0],
                    'permission' => 'stocks.show',
                    'types' => null,
                    'is_active' => false,
                ],
            ]
        ],
        'Cart' => [
            'title' => 'Cart',
            'icon' => 'bi bi-person-circle me-1',
            'route' => null,
            'parent_id' => null,
            'order' => 0,
            'parameters' => [],
            'permission' => 'carts',
            'types' => null,
            'is_active' => true,
            'children' => [
                'Cart List' => [
                    'title' => 'Cart List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'carts.index',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'carts.index',
                    'types' => null,
                    'is_active' => true,
                ],
                'Cart Create' => [
                    'title' => 'Cart Create',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'carts.create',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'carts.create',
                    'types' => null,
                    'is_active' => true,
                ],
                'Cart Edit' => [
                    'title' => 'Cart Edit',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'carts.edit',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['cart' => 0],
                    'permission' => 'carts.edit',
                    'types' => null,
                    'is_active' => false,
                ],
                'Cart Show' => [
                    'title' => 'Cart Show',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'carts.show',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['cart' => 0],
                    'permission' => 'carts.show',
                    'types' => null,
                    'is_active' => false,
                ],
                'Checkout' => [
                    'title' => 'Checkout',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'carts.checkout',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'carts.checkout',
                    'types' => null,
                    'is_active' => true,
                ],
            ]
        ],
        'Order' => [
            'title' => 'Order',
            'icon' => 'bi bi-person-circle me-1',
            'route' => null,
            'parent_id' => null,
            'order' => 0,
            'parameters' => [],
            'permission' => 'orders',
            'types' => null,
            'is_active' => true,
            'children' => [
                'Order List' => [
                    'title' => 'Order List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'orders.index-for-company',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'orders.index-for-company',
                    'types' => null,
                    'is_active' => true,
                ],
                'My Order List' => [
                    'title' => 'My Order List',
                    'icon' => 'bi bi-people me-1',
                    'route' => 'orders.index',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => [],
                    'permission' => 'orders.index',
                    'types' => null,
                    'is_active' => true,
                ],
                'Order Show' => [
                    'title' => 'Order Show',
                    'icon' => 'bi bi-person me-1',
                    'route' => 'orders.show',
                    // 'parent_id' => null,
                    'order' => 0,
                    'parameters' => ['order' => 0],
                    'permission' => 'orders.show',
                    'types' => null,
                    'is_active' => false,
                ],
            ]
        ],
    ],
];
