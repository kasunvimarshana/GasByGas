        ...
        <div class="card-body">
          ...
          <form method="POST"
            ...
            <div class="mb-3">
                ...
                <textarea name="address" class="form-control" rows="5" placeholder="{{ __('messages.address') }}">{{ old('address', $user->address) }}</textarea>
                @error('address')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                @if($user->image)
                <div class="mt-2">
                    <img src="{!! asset($user->image) !!}" alt="User Image" class="img-thumbnail" />
                </div>
                @endif
                @error('image')
                <div class="text-danger">{{ $message }}</div>
